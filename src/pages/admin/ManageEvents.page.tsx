...
import type { EventItem } from "@/services/firebase/types";
...
export const AdminManageEventsPage = () => {
	...
	const submitNewEvent: FormEventHandler = async (e) => {
		...
		if (!isAfter(parseISO(newEvent.endTime), parseISO(newEvent.startTime))) {
			toaster.error({
							...
							{events.map((evt) => (
								<li key={evt.id} className="bg-white p-6 shadow-lg rounded-lg">
									...
									<div className="mb-4">
										...
										<p>{format(evt.startTime, "MMM dd, yyyy (hh:mma)")}</p>
									</div>
									<div className="mb-4">
										<p className="font-semibold text-lg underline">End:</p>
										<p>{format(evt.endTime, "MMM dd, yyyy (hh:mma)")}</p>
									</div>
