...
const TeamNameRequestSchema = z.object({
	name: z.string().min(1),
});

const EmailsRequestSchema = z.object({
 ...
 */
export const isTeamNameAvailable = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}

	const data = TeamNameRequestSchema.parse(req.data);
	...
	try {
		...
		throw new HttpsError("internal", "Service down 1201");
	}
});
 ...
 */
export const createTeam = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	try {
		...
		if (snap.size < 1) {
			...
			throw new HttpsError(
				"permission-denied",
				"Not accepted into the hackathon.",
			);
		}
		...
		throw new HttpsError("internal", "Service down 1205");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Service down 1206");
	}
	...
	try {
		...
		if (team) {
			...
			throw new HttpsError("already-exists", "User already belongs to a team.");
		}
	} catch (e) {
		logError(
		...
		throw new HttpsError("internal", "Service down 1202");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Service down 1203");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Failed to create team. (3)");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Service down 1204");
	}
});
 ...
 */
export const getTeamByUser = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	try {
		...
		const team = await internalGetTeamByUser(req.auth.uid);
		if (!team) {
			throw new HttpsError("not-found", "No team found for requesting user.");
		}
		...
		const teamData: TeamData = {
			...
			members: totalMembers
				...
				.filter((m) => m.email !== req.auth?.token?.email),
		};
 ...
 */
export const inviteMember = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	if (process.env.TEAMS_ALLOW_INVITE_MEMBER !== "true") {
		throw new HttpsError(
			"permission-denied",
			"Invite member is not available.",
		);
	}
	...
	try {
		...
		team = await internalGetTeamByUser(req.auth.uid);
		if (!team) {
			logInfo(
		...
		throw new HttpsError("internal", "Service down 1207");
	}
	...
	if (team.owner !== req.auth.uid) {
		...
		throw new HttpsError(
			"permission-denied",
			"You must be the owner of the team to invite others.",
		);
	}
	...
	try {
		...
		throw new HttpsError(
			"not-found",
			"The email you have entered does not match.",
		);
	}

	if (!userRecord.customClaims?.rsvpVerified) {
		throw new HttpsError("not-found", "Could not send invitation.");
	}
	...
	try {
		...
		if (team) {
			throw new HttpsError("not-found", "Hacker already in another team.");
		}
	} catch (e) {
		logError(
		...
		throw new HttpsError("internal", "Service down (team-invitation)");
	}
	...
	try {
		...
		if (!app) {
			...
			throw new HttpsError("not-found", "Invalid request");
		}
	} catch (error) {
		logError("Failed to check if inviteee has been accpeted to HawkHacks.", {
			func,
		});
		throw new HttpsError("internal", "Service down 1208");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Failed to invite member to join team.");
	}
 ...
 */
export const updateTeamName = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	if (process.env.TEAMS_ALLOW_UPDATE_TEAM_NAME !== "true") {
		throw new HttpsError(
			"permission-denied",
			"Team name update is not available.",
		);
	}
	const data = TeamNameRequestSchema.parse(req.data);
	...
	try {
		...
		if (!team) {
			...
			throw new HttpsError("not-found", "Failed to find team.");
		}
		...
		throw new HttpsError("internal", "Failed to change team name (1).");
	}
 ...
 * with other teams.
 */
export const removeMembers = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	if (process.env.TEAMS_ALLOW_REMOVE_MEMBERS !== "true") {
		throw new HttpsError(
			"permission-denied",
			"Remove members is not available.",
		);
	}

	const data = EmailsRequestSchema.parse(req.data);
	...
	try {
		...
		throw new HttpsError("internal", "Failed to remove members (1).");
	}
 ...
 */
export const deleteTeam = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}

	if (process.env.TEAMS_ALLOW_DELETE_TEAM !== "true") {
		throw new HttpsError("permission-denied", "Team deletion not available.");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Failed to delete team (1).");
	}
 ...
 */
export const validateTeamInvitation = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}

	const data = CodeRequestSchema.parse(req.data);
	...
	try {
		...
		if (!invitation) {
			...
			throw new HttpsError(
				"not-found",
				"Invitation does not exists or expired.",
			);
		}
		...
		throw new HttpsError("internal", "Failed to join team. (2)");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Failed to join team.");
	}
 ...
 */
export const rejectInvitation = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Failed to reject invitation. (2)");
	}
...
export const checkInvitation = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Service Down (invitation) ");
	}
});

export const getUserInvitations = onCallCustom(async (req) => {
	if (!req.auth) {
		throw new HttpsError("permission-denied", "Unauthorized");
	}
	...
	try {
		...
		throw new HttpsError("internal", "Could not get user team invitations");
	}
