...
export const checkRSVP = functions
    .runWith({
    ...
    .https.onCall(async (_, context) => {
        ...
        functions.logger.info("Checking RSVP...", {
            uid: context.auth.uid,
            fn: "checkRSVP",
        });
        ...
        try {
            const rsvpRef = admin.firestore().collection("rsvps");
            const q = rsvpRef.where("uid", "==", context.auth.uid).limit(1);
            ...
            return { status: 200, verified: resource.data().verified };
        } catch (e) {
            functions.logger.error("Failed to check RSVP", {
                uid: context.auth.uid,
                cause: (e as Error).message,
            });
            throw new functions.https.HttpsError(
                "internal",
                "Failed to check RSVP"
            );
        }
    });
